...
export const initializeQuantityControls = (btnProductId) => {
  ...
  if (increaseBtn) {
    ...
    increaseBtn.increaseHandler = function () {
      let quantityInput = this.parentElement.querySelector(
        `[btn_product_id="${btnProductId}"]`
      );
  ...
  if (decreaseBtn) {
    ...
    decreaseBtn.decreaseHandler = function () {
      ...
      console.log(
        "Decrease button clicked, current value:",
        quantityInput?.value
      );
      ...
      if (quantityInput && parseInt(quantityInput.value) >= 1) {
        ...
        console.log(
          "Cannot decrease: quantity is already 0 or input not found"
        );
      }
    };
      ...
      btn.addEventListener("click", async () => {
        if (btn.querySelector(".buy-button")) {
          ...
          const input = document.querySelector(
            `input[btn_product_id="${btnProductId}"]`
          );
